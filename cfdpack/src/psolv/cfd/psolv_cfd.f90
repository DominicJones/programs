...
!   integer :: i, ii, j, jj1, jjn, k, m, n
  ...
  real(dp) :: dnum
  real(rp) :: cc, cd, summ, res0(10), gl_res0, time(2)
...
!   print "(/a,1x,i0,1x,a,1x,i0/)", "proc", blk, "of", n_blk
!   return
...
!   xdmn%x_vrt => geom_x_vrt
  ...
  if (blk == 0) print "(a,1x,g12.4)","domain volume:", xdmn%vol_sum
  ...
  vel%calc_spd = 1
  vel%urf = 0.55; vel%bl_c = 0.5; vel%bl_d = 1
  cc = 1 ! density
  cd = cc / 1000 ! viscosity
  vel%spd = zero
...
!   call random_number (vel%phi%dmn)
  ...
  pv_scheme = 1; pp%urf = 0.15
!   pv_scheme = 2; pp%urf = 0.9
  pp%ref = zero
  ...
  do i = 1, dmn%n_fce
    ...
    if (jn < 0) then
      ...
      if (ph_ty == 31) then
        vel%phi%bnd(1,jbl) = 1.00
      end if

      if (ph_ty == 11) then
        ! vel%phi%bnd(:,jbl) = -xdmn%norm(:,i) * 1.00
      end if
    end if
  end do
  print *, "BC: Cavity"
...
!     end do
    ...
    if (blk == 0) then
      call fp_iter_ctrl('F', 1, iter, n_iter, 1, &
        fp_exit, ten**(-5), res0, gl_res0)
    end if
...
subroutine write_fields ()
  ...
  ! call write_gmsh_field (f_len, out_file, dmn%n_elm*dmn%n_dim, dmn%elm_idx, vel%phi%dmn)
