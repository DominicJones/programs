...
C     SIMPLE algorithm on colocated body-fitted grids. The
C     grid non-orthogonality is taken into account in the
C     pressure-correction equation. Also, the non-smoothness
C     of grid lines is taken into account. For a description
C     of the solution method, see Chap. 8 of the book by
...
C
C     This is Version 1.4 of the code, November 1997.
C
...
C
C.....ITERATE FOR P' (ONE FOR ORTHOGONAL AND MORE FOR NON-ORTH.)    
C
      DO LC=1,NPCOR
C
...
C     separate loops...
C     Due to the fact that e' and e locations - see Fig. 8.6 -
C     may not coincide, the gradient is calculated iterartively:
C     in the first step, the old gradients DFXO, DFYO are zero,
C     so the first gradient approximation is obtained using
C     variable values at e' as if they were at e; in the 
C     following steps, gradients from the previous step are
C     used to calculate the value at e using value and gradients
C     at e'. This produces smooth pressure field even if the
C     grid is 'kinky'.
C===============================================================
      INCLUDE 'param.inc'
      INCLUDE 'indexc.inc'
...
C================================================================
      ...
      DIMENSION FI(NXYA)
C
C.....INTERPOLATED CELL FACE VALUES (for the location e'; see
C     FLUXUV for the possibility of correction from e' to e).
C
      FACP=1.-FAC
