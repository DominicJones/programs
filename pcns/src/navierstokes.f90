...
!   integer :: i, ii, j, jj1, jjn, k, m, n
  ...
  real(dp) :: dnum, bl_c, bl_d, vel_urf, pp_urf
  ...
  integer :: ii, m, n, nnz, ipp, n_pp, n_pp_mod
  ...
  integer :: f_len
  integer :: write_freq, write_pres, write_vel, write_spd, write_wall_force
  character(sl) :: out_file
  type(sfld_t) :: area
  integer,dimension(:),allocatable::bvrt_idx

  character(len=256):: bnd_type, meshfile
  integer :: bnd_id
  ...
  call fson_get(json, "Mesh.File", meshfile)
  ...
  call fson_get(json, "Solver.Green-Gauss-Iterations", n_gg)
  call fson_get(json, "Solver.Pressure-Correction-Iterations", n_pp)
  call fson_get(json, "Solver.Pressure-Velocity-Scheme", pv_scheme)
  call fson_get(json, "Equation.Velocity.Relaxation", vel%urf)
  call fson_get(json, "Equation.Velocity.Iteration-Limit", vel%n_iter)
  call fson_get(json, "Equation.Velocity.Residual-Drop", vel%reduc)
  call fson_get(json, "Equation.Pressure.Relaxation", pp%urf)
  call fson_get(json, "Equation.Pressure.Iteration-Limit", pp%n_iter)
  call fson_get(json, "Equation.Pressure.Residual-Drop", pp%reduc)
  call fson_get(json, "Model.Navier-Stokes.Convection-Blending", vel%bl_c)
  call fson_get(json, "Model.Navier-Stokes.Diffusion-Blending", vel%bl_d)
  ...
  call fson_get(json, "Output.Frequency", write_freq)
  call fson_get(json, "Output.Pressure", write_pres)
  call fson_get(json, "Output.Velocity", write_vel)
  call fson_get(json, "Output.Speed", write_spd)
  call fson_get(json, "Output.Wall-Force", write_wall_force)
...
!   xdmn%x_vrt => geom_x_vrt
  ...
  ! call print_range("bnd%elm_blk [f90]", bnd%n_blk, bnd%elm_blk);
  ! call print_range("dmn%elm_blk [f90]", dmn%n_blk, dmn%elm_blk);
  ! call print_graph("dmn%fce_elm [f90]", dmn%n_fce, dmn%fce_elm%row, dmn%fce_elm%col);
...
!   call random_number (vel%phi%dmn)
  ...
  do iter = 1, n_iter
     ...
     ! no high-order interpolation for first block of iterations
     if (iter > write_freq) then
        vel%bl_c = bl_c;
        vel%bl_d = bl_d;
     else
        bl_c = vel%bl_c; vel%bl_c = 0;
        bl_d = vel%bl_d; vel%bl_d = 0;
     end if
    ...
    ! pressure gradient
    if (n_gg == 0) call ls_gradient (bnd, dmn, xbnd, xdmn, pres%phi, pres%grad, 1)
    if (n_gg > 0) call gg_gradient (bnd, dmn, xbnd, xdmn, pres%phi, pres%grad, 1, n_gg)
#ifdef USE_MPI
    ...
    ! velocity gradient
    if (n_gg == 0) call ls_gradient (bnd, dmn, xbnd, xdmn, vel%phi, vel%grad, 0)
    if (n_gg > 0) call gg_gradient (bnd, dmn, xbnd, xdmn, vel%phi, vel%grad, 0, n_gg)
#ifdef USE_MPI
    ...
    do i = 1, vel%n_cmp
      ...
      call lin_eqns_solver ( &
        ...
        res0(i),vel%mat%col,vel%mat%row,vel%n_iter,vel%reduc,2,i)
#endif
    end do
    ...
    do ipp = 1, n_pp
      ...
      call lin_eqns_solver ( &
        ...
        res0(i),vel%mat%col,vel%mat%row,pp%n_iter,pp%reduc,1,i)
#endif
      ...
      if (n_gg == 0) call ls_gradient (bnd, dmn, xbnd, xdmn, pp%phi, pp%grad, 1)
      if (n_gg > 0) call gg_gradient (bnd, dmn, xbnd, xdmn, pp%phi, pp%grad, 1, n_gg)
#ifdef USE_MPI
      ...
      pp%rhs = zero
      call correct_pp (bnd, dmn, xbnd, xdmn, vel, pres, pp)
    end do


    if (blk == 0) then
      call fp_iter_ctrl(iter, n_iter, fp_exit, res_drop, res0, gl_res0)
    end if

    if (((write_freq > 0) .and. mod(iter, write_freq) == 0)) then
       call write_fields ()
    end if
    ...
    if ((iter > write_freq) .and. (fp_exit > 0)) exit
  end do
...
subroutine write_fields ()
  if (write_vel == 1) then
     out_file = trim(dir(filename))//trim(rm_ext(meshfile))//"_vel.msh"
     if (n_blk > 1) then
  ...
  if (write_spd == 1) then
     out_file = trim(dir(filename))//trim(rm_ext(meshfile))//"_spd.msh"
     if (n_blk > 1) then
        out_file = trim(out_file)//"_p"//trim(itoa(blk))
     end if
     f_len = len_trim(out_file)
  ...
  if (write_pres == 1) then
     out_file = trim(dir(filename))//trim(rm_ext(meshfile))//"_pres.msh"
     if (n_blk > 1) then
        out_file = trim(out_file)//"_p"//trim(itoa(blk))
     end if
     f_len = len_trim(out_file)
  ...
  if (write_wall_force > 0) then
     call wall_force(write_wall_force);
  end if
end subroutine
    ...
    if (jn < 0) then
      ...
      if (ph_ty == bnd_id) then
         if (vec_mag(bnd_value(1:3)) < 1.0e-20) then
            ...
            if (bnd%n_dim == 3) vel%phi%bnd(3,jbl) = bnd_value(3) * bnd_value(4)
         end if
      end if
    end if
  end do
...
subroutine wall_force(wall_id)
  integer,intent(in)::wall_id

  integer :: bnd1,bndl,bndu,dmn1,dmnl,dmnu
  integer :: i,j,k,kii,kij,kji,kjj,ic,ib,ph_ty
  integer :: j1, j11, j1l, jb, jb1, jbl, jn, jnl, jj1, jjn, jj
  integer :: bnd_os
  character(len=256):: wall_type

  real(rp)::d,fpres,area,l_pn,cd
  real(rp),dimension(mesh_n_dim)::n,t,f_shear,f_pres,vvel,fvel,f_wall

  f_wall=0; f_pres=0; f_shear=0;

  wall_type = "Wall";
  call boundary_offset(wall_type, bnd_os)
  ...
  do i = 1, dmn%n_fce
    ...
    if (jn < 0) then
      ...
      if (ph_ty == (wall_id + bnd_os)) then
         ...
         d = cd * area/l_pn
         vvel = vel%phi%dmn(:,j1l)
         fvel = vel%phi%bnd(:,jbl)
         fpres = pres%phi%bnd(1,jbl)

         t = fvel+vvel
         t = t - n*dot_product(t,n)
         t = t / (vec_mag(t)+small)

         f_shear = -d*(fvel-t*dot_product(vvel,t))
         f_pres = fpres*n * area
  ...
  if (mesh_n_dim == 2) then
     print "(a,i1,a,2es11.3,a)", "Wall (",wall_id,") force = [", f_wall,"]"
  else
     print "(a,i1,a,3es11.3,a)", "Wall (",wall_id,") force = [", f_wall,"]"
  end if
end subroutine
